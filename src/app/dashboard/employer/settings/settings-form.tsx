...
import { updateEmployerProfile } from "@/app/action/profile";
...
export function EmployerSettingsForm({ email, profile }: SettingsFormProps) {
  ...
  const [recruiterTitle, setRecruiterTitle] = useState(profile.recruiterTitle ?? "");
  ...
  function saveProfile(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await updateEmployerProfile({ organizationName, organizationWebsite, recruiterName, recruiterTitle });
      if (!result.success) {
      ...
      setOrganizationWebsite(result.data.organization_website ?? "");
      setRecruiterName(result.data.recruiter_name ?? "");
      setRecruiterTitle(result.data.recruiter_title ?? "");
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-12">
      <header className="max-w-2xl">
        ...
        <p className="mt-2 text-sm leading-5 text-text-secondary">Manage your employer profile, account, and sign-in details.</p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
        <section aria-labelledby="profile-heading" className="rounded-2xl border border-border bg-surface p-5 shadow-[0_1px_0_rgba(17,18,17,0.02)] sm:p-6">
          ...
          <div className="mt-2">
            <h2 id="profile-heading" className="text-sm font-bold tracking-tight text-foreground">Company information</h2>
            <p className="mt-1.5 text-sm text-text-secondary">Details candidates see when interacting with your organization.</p>
          </div>

          <form onSubmit={saveProfile} className="mt-5">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="organization-name" className="text-xs font-semibold text-foreground">Company name</label>
                <Input id="organization-name" value={organizationName} onChange={(event) => setOrganizationName(event.target.value)} maxLength={150} autoComplete="organization" className="mt-1.5 bg-background/45" />
              </div>
              <div>
                <label htmlFor="organization-website" className="text-xs font-semibold text-foreground">Company website</label>
                <Input id="organization-website" type="url" value={organizationWebsite} onChange={(event) => setOrganizationWebsite(event.target.value)} maxLength={200} placeholder="https://example.com" className="mt-1.5 bg-background/45" />
              </div>
            </div>

            <div className="mt-7 border-t border-border pt-5">
              <h3 className="text-sm font-bold tracking-tight text-foreground">Recruiter information</h3>
              <p className="mt-1.5 text-sm text-text-secondary">Your identity as the person managing hiring.</p>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="recruiter-name" className="text-xs font-semibold text-foreground">Full name</label>
                  <Input id="recruiter-name" value={recruiterName} onChange={(event) => setRecruiterName(event.target.value)} maxLength={120} autoComplete="name" className="mt-1.5 bg-background/45" />
                </div>
                <div>
                  <label htmlFor="recruiter-title" className="text-xs font-semibold text-foreground">Job title</label>
                  <Input id="recruiter-title" value={recruiterTitle} onChange={(event) => setRecruiterTitle(event.target.value)} maxLength={120} placeholder="Talent Acquisition" className="mt-1.5 bg-background/45" />
                </div>
              </div>
            </div>
